  ...
  async read(userId: string, companyId: string): Promise<ExecutiveAuditResult | null> {
    ...
    const [evidenceLinks, connectorLinks, detectionRules] = await Promise.all([
      this.db.automationOpportunityEvidence.findMany({
        where: { organizationId, snapshotId: automationRef.id },
      }),
    ...
    const [facts, evidenceRecords] = knowledgeRef
      ? await Promise.all([
          this.db.knowledgeFact.findMany({
            where: { organizationId, snapshotId: knowledgeRef.id, id: { in: factIds } },
          }),
          this.db.knowledgeEvidence.findMany({
            where: { organizationId, snapshotId: knowledgeRef.id, factId: { in: factIds } },
          }),
        ])
      : [[], []];
    const safetyFor = (opportunity: (typeof opportunities)[number]): OpportunityDecisionSafety => {
      const links = evidenceLinks.filter(
        (link) =>
          ...
          findings.some((finding) => finding.id === link.businessFindingId),
      );
      const linkedFacts = facts.filter((fact) =>
        links.some((link) => link.knowledgeFactId === fact.id),
      );
      const qualityFor = (fact: (typeof facts)[number]): DecisionEvidenceQuality => {
        ...
        if (evidence.some((record) => /contradict/i.test(record.evidenceType)))
          return "CONTRADICTORY";
        if (evidence.some((record) => /assum/i.test(record.evidenceType))) return "ASSUMED";
        return Number(fact.confidencePercentage) >= 70 &&
          evidence.every(
            (record) =>
              ["validated_entity", "validated_answer"].includes(record.evidenceType) &&
              Number(record.confidencePercentage) >= 70,
          )
          ? "SUPPORTED"
          : "INFERRED";
      };
      ...
      if (!required || required.length !== connectors.length)
        prerequisites.push({
          id: "connector_checklist",
          opportunityId: opportunity.id,
          kind: "connector",
          satisfied: null,
          remediation: "Compléter la liste des connecteurs requis et vérifier leur disponibilité.",
        });
      if (
        linkedFacts.some((fact) => ["finance", "hr", "legal", "compliance"].includes(fact.domain))
      )
        prerequisites.push({
          id: "human_approval",
          opportunityId: opportunity.id,
      ...
      return {
        ...
        companyId,
        opportunityId: opportunity.id,
        automationSnapshotId: automationRef.id,
        evidence: linkedFacts.map((fact) => ({ id: fact.id, quality: qualityFor(fact) })),
        observations: linkedFacts.flatMap((fact) =>
          typeof fact.valueJson === "string" ||
          typeof fact.valueJson === "number" ||
          typeof fact.valueJson === "boolean"
            ? [
                {
                  factId: fact.id,
                  key: fact.factKey,
                  value: fact.valueJson,
                  quality: qualityFor(fact),
                },
              ]
            : [],
        ),
