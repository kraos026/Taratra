...
export async function POST(r: Request, c: C) {
  const id = idSchema.safeParse((await c.params).id),
    input = sectionInputSchema.safeParse(await r.json().catch(() => null));
  if (!id.success || !input.success) return questionnaireValidationError();
  return withQuestionnaireService("questionnaireSections.create", (s) =>
    s.addSection(id.data, input.data).then((v) => apiSuccess(v, 201)),
  );
}
