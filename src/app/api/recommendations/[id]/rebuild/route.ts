...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id.success || !body.success) return apiError("VALIDATION_ERROR", "Invalid rebuild", 400);
  return withRecommendationPortfolioService((service) =>
    service.rebuild(id.data, body.data.lockVersion).then((value) => apiSuccess(value, 201)),
  );
}
