...
import { configureSystemChromeForPlaywright } from "../../scripts/system-chrome.mjs";

const PREVIEW_URL = "https://taratra-h3v6qbar2-optivos.vercel.app";
...
const audit = {
  companyName: `Optivos Pilot Company ${certificationRunId}`,
...
async function login(page) {
  ...
  await page.getByLabel(/email/i).fill(stagingUser("A").email);
  await page.getByLabel(/password|mot de passe/i).fill(stagingUser("A").password);
...
async function ensureCompany(page) {
  const created = await api(page, "/api/companies", {
    ...
    body: {
      ...
      country: "France",
      description:
        "Pilot tenant used to validate the Optivos audit flow with invoice processing evidence.",
      status: "client",
    },
  });
...
async function tenantUser(label) {
  const email = stagingUser(label).email;
  const row = await one(
...
function stagingUser(label) {
  const prefix = label === "A" ? "AUTOMATEX_STAGING_TENANT_A" : "AUTOMATEX_STAGING_TENANT_B";
  const email = process.env[`${prefix}_EMAIL`]?.trim();
  const password = process.env[`${prefix}_PASSWORD`]?.trim();
  if (!email || !password) throw new Error(`Tenant ${label} staging credentials are missing`);
...
function requireStagingEnv() {
  ...
  if (supabaseUrl.hostname !== "ajvncwsazrhqjlktzojm.supabase.co")
    throw new Error("NEXT_PUBLIC_SUPABASE_URL does not target staging");
}
...
async function main() {
  loadDotEnvLocal();
  requireStagingEnv();
  configureSystemChromeForPlaywright(process.env);

  console.log("TARGET = automatex-staging");
  ...
  browser = await chromium.launch({
    ...
    executablePath: process.env.AUTOMATEX_SYSTEM_CHROME_PATH,
    headless: true,
  });
